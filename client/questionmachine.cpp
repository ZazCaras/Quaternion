...
void QuestionMachine::procesar(std::string mensaje) {
    switch(_estado) {
        ...
        case Pregunta:
            if (mensaje.rfind("Emitir pregunta: ", 0) == 0 || mensaje.rfind("Emitir Pregunta: ", 0) == 0) {
                _pregunta = mensaje.substr(17);
                _estado = Respuestas;
            }
            break;
        case Respuestas:
            if (mensaje.rfind("Responder pregunta: ", 0) == 0 || mensaje.rfind("Responder Pregunta: ", 0) == 0) {
                std::string respuesta = mensaje.substr(20);

                _respuestas.push_back(respuesta);
            } else if (mensaje.rfind("Finalizar pregunta", 0) == 0 || mensaje.rfind("Finalizar Pregunta", 0) == 0) {
                imprimirTodo();
